...
// launch. Left OUT on purpose vs. the original guion PDF: the Early Bird/
// Regular/Full price-tier language and the "el precio sube en cada etapa"
...
// only payment method actually wired up so far — see docs/PAYMENTS_SETUP.md).
// Both can be added back into this same text once those features exist.
//
...
async function main() {
  ...
  await db.event.update({
    where: { slug },
    data: { description: sanitizeEventDescription(SALES_PAGE_HTML) },
  });

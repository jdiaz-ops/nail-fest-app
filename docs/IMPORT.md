# Importar registros históricos

`/admin/import` sube el export "doorlist" de un evento (CSV) y lo
convierte en Person + Registration + Consent en el CRM — sin enviar
correos, sin generar QR, sin eventos de Meta CAPI. Es solo para poblar el
CRM y las audiencias de Meta con datos que ya existen de eventos pasados
(o presentes, como Cali 2025/2026, Manizales, Armenia, Bogotá, etc.), un
evento a la vez.

## Cómo conseguir el archivo

En la plataforma de ticketing que usábamos antes de este sistema: tu
evento → **Attendees** (o **Door list**) → **Export**. Ese export trae,
por ticket: nombre, tipo de boleta, código de ticket, Order ID, check-in
(Sí/No), email, teléfono, cédula/NIT, ciudad, profesión, Instagram — las
mismas preguntas del formulario de registro.

## Qué hace el import

1. **Todo el parsing pasa en tu navegador** (`lib/import/doorlistCsv.ts`)
   ...
   confirmar.
2. **Agrupa por email** — este formato de doorlist permite hasta 2
   boletas por orden (titular + un acompañante), pero el acompañante
   nunca tiene datos propios: ambas filas traen el email/teléfono del
   titular. Agrupar por email es correcto, no solo dedup.
3. **Aforo real (`ticketCount` / `checkedInCount`)** — cada registro guarda
   ...
   estado "Checked in" que muestra ese export — ver la tabla de aforo en
   ...
   **Límite real, no de esta app**: esto es un snapshot (boleta escaneada
   sí/no), no un log de reingresos — este formato de export no registra
   cuántas veces se escaneó cada boleta ni cuándo, solo si se escaneó
   alguna vez. Reingreso de verdad (titular entra sábado, sale, vuelve
   domingo) solo se podrá trackear con la futura app de escaneo propia,
   que si va a guardar cada escaneo como su propio evento con fecha/hora.

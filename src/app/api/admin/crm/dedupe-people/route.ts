...
// turning the phone-duplicate signal from duplicate-check/route.ts into
// an actual (conservative) decision — see personDedupe.ts's own comment
// for the exact matching rules. Two modes on the same computation:
...
// Groups that don't fully resolve into one cluster are reported as
// "needs review" and never written — same "report, don't guess" posture
// as every other tool on this page.
const REVIEW_LIST_LIMIT = 200;
const CHUNK_SIZE = 500;

// A real list can have thousands of merge writes across chunked
// transactions — same posture as email-quality's own maxDuration bump.
...
export async function POST(req: NextRequest) {
  ...
  if (apply && plan.merges.length > 0) {
    ...
    for (let i = 0; i < plan.merges.length; i += CHUNK_SIZE) {
      ...
      // Each entry can have its own canonicalId, so this can't be a
      // single `updateMany` with one shared value — a small
      // transaction per chunk keeps it from being one round trip per row.

...
export default function TermsAndConditionsForm({ initialText }: { initialText: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus("saving");
    try {
      await postSettings({ termsAndConditionsText: text });
      setStatus("saved");
    } catch {
      setStatus("error");
    }
  ...
  return (
    <form onSubmit={handleSubmit} style={cardStyle}>
      <div className="field">
        <label htmlFor="termsText">Texto (texto plano — párrafos separados por línea en blanco)</label>
        <textarea
          id="termsText"
          value={text}
